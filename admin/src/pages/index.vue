  ...
  <div class="container">
    ...
    <div class="tabs">
      ...
      background-color="#545c64">
        ...
        <el-tab-pane label="💡 网络控灯" name="3"><RouterView v-if="tabValue=='3'" v-bind="{...lampDeviceStatus}"/></el-tab-pane>
        ...
        <el-tab-pane label="🌱 智慧农场" name="7"><RouterView v-if="tabValue=='7'" v-bind="{...farmDeviceNum}"/></el-tab-pane>
      </el-tabs>
    </div>
    <div class="acc">
...
let lampDeviceStatus = ref({
  curtainStatus:false
})
let farmDeviceNum = ref({
  wetNum: 0,
  rayNum: 0,
  temNum:0
})
onMounted(() => {
  ...
  ws_switch.onmessage = (e) => {
      console.log('硬件开关我收到信息了',e);
      let data = JSON.parse(e.data)
      if(data.type == 1) {
        if (data.status === 0) {
            lampDeviceStatus.value.curtainStatus = false
        } else if (data.status === 1) {
            lampDeviceStatus.value.curtainStatus = true
        }
      }
      if (data.type == 2) {
  ...
  ws_num.onmessage = (e) => {
      ...
      if (data.type == 1) {
        ...
        farmDeviceNum.value.wetNum = data.value
      }else if (data.type == 4) {
        farmDeviceNum.value.temNum = data.value
      }else if (data.type == 5) {
        farmDeviceNum.value.rayNum = data.value
      }
  }
})
function changeTab() {
